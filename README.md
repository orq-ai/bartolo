...
- `doctor`, `auth list-profiles`, and the OAuth token cache all resolve the active profile through one function, so they cannot disagree about which profile is in force.
