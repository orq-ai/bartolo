// Code generated by bartolo. DO NOT EDIT.
// See https://github.com/orq-ai/bartolo

package generated

import (
	"fmt"
	"strings"

	neturl "net/url"

	bartolocli "github.com/orq-ai/bartolo/cli"
	"github.com/pkg/errors"
	"github.com/spf13/viper"
	"gopkg.in/h2non/gentleman.v2"
)

func servers() []map[string]string {
	return []map[string]string{

		map[string]string{
			"description": "",
			"url":         "https://api.example.com",
		},
	}
}

func initGeneratedRuntime() {
	bartolocli.RegisterServers(servers())

}

// ExampleGetThing Get a thing
func ExampleGetThing(paramId string, paramFrom string, params *viper.Viper) (*gentleman.Response, interface{}, error) {
	handlerPath := "get-thing id from"
	server := bartolocli.ResolveServer()

	url := server + "/{id}"
	if paramId == "" {
		return nil, nil, bartolocli.NewValueError(errors.Errorf("path parameter id cannot be empty"))
	}

	url = strings.Replace(url, "{id}", neturl.PathEscape(paramId), 1)

	if err := bartolocli.CheckParam("argument id", paramId, "uuid", []string{}); err != nil {
		return nil, nil, err
	}
	paramFromTime, err := bartolocli.NormalizeParam("argument from", paramFrom, "date-time", []string{})
	if err != nil {
		return nil, nil, err
	}
	paramFrom = paramFromTime

	req := bartolocli.Client.Get().URL(url)

	req = req.AddQuery("from", paramFrom)

	paramDetailed := params.GetBool("detailed")
	if bartolocli.FlagPassed(params, "detailed") || paramDetailed != false {
		req = req.AddQuery("detailed", fmt.Sprintf("%v", paramDetailed))
	}
	paramLimit := params.GetInt64("limit")
	if bartolocli.FlagPassed(params, "limit") || paramLimit != 0 {
		req = req.AddQuery("limit", fmt.Sprintf("%v", paramLimit))
	}
	paramRatio := params.GetFloat64("ratio")
	if bartolocli.FlagPassed(params, "ratio") || paramRatio != 0.0 {
		req = req.AddQuery("ratio", fmt.Sprintf("%v", paramRatio))
	}
	paramKind := params.GetString("kind")
	if bartolocli.FlagPassed(params, "kind") || paramKind != "" {
		if err := bartolocli.CheckParam("--kind", paramKind, "", []string{"internal", "a2a"}); err != nil {
			return nil, nil, err
		}
		req = req.AddQuery("kind", fmt.Sprintf("%v", paramKind))
	}
	paramTo := params.GetString("to")
	if bartolocli.FlagPassed(params, "to") || paramTo != "" {
		paramToTime, err := bartolocli.NormalizeParam("--to", paramTo, "date-time", []string{})
		if err != nil {
			return nil, nil, err
		}
		paramTo = paramToTime
		req = req.AddQuery("to", fmt.Sprintf("%v", paramTo))
	}
	paramCursor := params.GetString("cursor")
	if bartolocli.FlagPassed(params, "cursor") || paramCursor != "" {
		req = req.AddQuery("cursor", fmt.Sprintf("%v", paramCursor))
	}
	paramSess := params.GetString("sess")
	if bartolocli.FlagPassed(params, "sess") || paramSess != "" {
		if err := bartolocli.CheckParam("--sess", paramSess, "", []string{"a", "b"}); err != nil {
			return nil, nil, err
		}
	}

	bartolocli.HandleBefore(handlerPath, params, req)

	resp, err := req.Do()
	if err != nil {
		return nil, nil, errors.Wrap(err, "request failed")
	}

	var decoded interface{}

	if resp.StatusCode < 400 {
		if err := bartolocli.UnmarshalResponse(resp, &decoded); err != nil {
			return nil, nil, errors.Wrap(err, "unmarshalling response failed")
		}
	} else {
		return nil, nil, errors.Errorf("HTTP %d: %s", resp.StatusCode, resp.String())
	}

	after := bartolocli.HandleAfter(handlerPath, params, resp, decoded)
	if after != nil {
		decoded = after
	}

	return resp, decoded, nil
}

// ExampleCreateWidget Create a widget
func ExampleCreateWidget(params *viper.Viper, body string) (*gentleman.Response, interface{}, error) {
	handlerPath := "widgets create"
	server := bartolocli.ResolveServer()

	url := server + "/widgets"

	req := bartolocli.Client.Post().URL(url)

	if body != "" {
		req = req.AddHeader("Content-Type", "application/json").BodyString(body)
	}

	bartolocli.HandleBefore(handlerPath, params, req)

	resp, err := req.Do()
	if err != nil {
		return nil, nil, errors.Wrap(err, "request failed")
	}

	var decoded interface{}

	if resp.StatusCode < 400 {
		if err := bartolocli.UnmarshalResponse(resp, &decoded); err != nil {
			return nil, nil, errors.Wrap(err, "unmarshalling response failed")
		}
	} else {
		return nil, nil, errors.Errorf("HTTP %d: %s", resp.StatusCode, resp.String())
	}

	after := bartolocli.HandleAfter(handlerPath, params, resp, decoded)
	if after != nil {
		decoded = after
	}

	return resp, decoded, nil
}

// ExampleDeleteWidget Delete a widget
func ExampleDeleteWidget(paramId string, params *viper.Viper) (*gentleman.Response, interface{}, error) {
	handlerPath := "widgets delete id"
	server := bartolocli.ResolveServer()

	url := server + "/widgets/{id}"
	if paramId == "" {
		return nil, nil, bartolocli.NewValueError(errors.Errorf("path parameter id cannot be empty"))
	}

	url = strings.Replace(url, "{id}", neturl.PathEscape(paramId), 1)

	req := bartolocli.Client.Delete().URL(url)

	bartolocli.HandleBefore(handlerPath, params, req)

	resp, err := req.Do()
	if err != nil {
		return nil, nil, errors.Wrap(err, "request failed")
	}

	var decoded interface{}

	if resp.StatusCode < 400 {
		if err := bartolocli.UnmarshalResponse(resp, &decoded); err != nil {
			return nil, nil, errors.Wrap(err, "unmarshalling response failed")
		}
	} else {
		return nil, nil, errors.Errorf("HTTP %d: %s", resp.StatusCode, resp.String())
	}

	after := bartolocli.HandleAfter(handlerPath, params, resp, decoded)
	if after != nil {
		decoded = after
	}

	return resp, decoded, nil
}
