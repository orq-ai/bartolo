...
- **The credentials file is written `0600`.** `viper.WriteConfigAs` left the long-lived API-key file world-readable at `0644`; the file is now created `0600` before the key is written, and an existing `0644` file is narrowed. Unix only: Go's `os.Chmod` on Windows toggles the read-only attribute and nothing else, so there the file keeps whatever its directory's ACL grants.
- **API keys are no longer required as positional args.** `auth add-profile` prompts for each secret without echo instead of taking it on the command line (where it lands in shell history and `ps`). Positional values still work for backward compatibility but are discouraged, and the generated README no longer suggests them.
...
- **Destructive `delete` commands now confirm.** Generated delete commands prompt before running and gain a `-f, --force` flag; a non-interactive shell without `--force` refuses rather than deleting silently. The prompt names the target (`widgets delete abc`), the gate runs after `--example`, which prints without sending a request, and a prompt that cannot be read exits `1` rather than reporting a delete that never happened as a success. `-f` is only bound when it is free, since a consumer can claim it first through `cli.AddFlag` and pflag panics on a duplicate shorthand.
...
- **`auth add-profile` no longer hangs in CI.** A missing key value prompted whenever the positional argument was absent, and `survey` reads stdin directly, so an open-but-idle pipe blocked forever. The terminal check now happens before the prompt, so a pipe exits `2` with the usage message.
