...
- **`auth add-profile` no longer hangs in CI.** A missing key value prompted whenever the positional argument was absent, and `survey` reads stdin directly, so an open-but-idle pipe blocked forever. The terminal check now happens before the prompt, so a pipe exits `2` with the usage message.
  ...
  **Note:** `force` is reserved on every generated command, not just `Delete` ones, so a spec parameter or body field named `force` is exposed as `--param-force` / `--body-force` even where no `--force` flag exists to collide with.
