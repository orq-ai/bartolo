...
Releases are cut automatically from conventional-commit subjects once CI passes
on `main`, and the release notes are generated on the GitHub Release. Entries
below are kept for the versions that predate that.
...
- **v0.4.7 and v0.4.8 were never tagged or released** — the constant was bumped
  twice but no tag exists for either, so `go install ...@latest` never saw them.
  Everything under Unreleased and here ships in the first automatic release
  instead. This is the drift the automatic releases exist to prevent.
